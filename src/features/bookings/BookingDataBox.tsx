...
import { BookingDataBoxInfo } from "../../types/types";
...
function BookingDataBox({ booking }: { booking: BookingDataBoxInfo }) {
	const {
		created_at,
		startDate,
		endDate,
	...
	if (!startDate || !endDate || !numGuests || !totalPrice || !cabinPrice || !created_at) throw new Error("Booking info incomplete");
			...
			<Section>
				...
				<Price $isPaid={isPaid ?? false}>
					<DataItem icon={<HiOutlineCurrencyDollar />} label={`Total price`}>
						...
						{hasBreakfast && ` (${formatCurrency(cabinPrice)} cabin + ${formatCurrency(extrasPrice ?? 0)} breakfast)`}
					</DataItem>
